...
- **MT5 worker must LAUNCH its own terminal — never attach to a manually-opened one**: `_connect_mt5()` calls `mt5.initialize(MT5_TERMINAL_PATH, login=…, password=…, server=…)`. On the Windows VPS the Python↔MT5 IPC only works for a terminal the library started itself; attaching to a terminal opened by hand fails with `(-10005, 'IPC timeout')` (no terminal info, no login attempted — happens *before* credentials are used, so it is never a password/server problem). **Operational rule: ensure NO `terminal64.exe` is running before starting the worker, then `uv run python layer3/worker_personal.py` — the worker opens MT5 and logs into the correct account automatically.** Do not auto-start MT5 on boot. `MT5_TERMINAL_PATH` defaults to `C:\Program Files\MetaTrader 5\terminal64.exe`; override in `.env` if installed elsewhere. Root cause of the 2026-05-24 live-cutover IPC-timeout incident.
