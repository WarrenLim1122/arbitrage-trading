...
- `baseline_equity` and `pers_baseline_equity` are **immutable** — only written by deliberate operator action: the `/changepropfirm` wizard, the `/phase2` wizard, or the `/setbaseline <prop|personal> <amount>` quick-fix command. Never auto-set from MT5 balance.
