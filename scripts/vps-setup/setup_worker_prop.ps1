# Run this in PowerShell as Administrator on worker-prop (45.76.156.55)
...
# 6. Write .env file (fill in your FundingPips MT5 credentials below)
@"
WORKER_NAME=prop
MT5_LOGIN=REPLACE_WITH_FUNDINGPIPS_ACCOUNT_NUMBER
MT5_PASSWORD=REPLACE_WITH_FUNDINGPIPS_PASSWORD
MT5_SERVER=REPLACE_WITH_FUNDINGPIPS_SERVER
...
Write-Host ""
Write-Host "=== worker-prop setup complete ===" -ForegroundColor Green
Write-Host "Next: install MT5 from FundingPips, log in, enable automated trading." -ForegroundColor Yellow
Write-Host "Then edit C:\arbitrage\.env and fill in your MT5 credentials." -ForegroundColor Yellow
Write-Host "Then run: cd C:\arbitrage && uv run python layer3/worker_prop.py" -ForegroundColor Yellow
